...
#include <iostream>     
#include <cmath>        
#include <random>       //βιβλιοθήκη για τυχαίους αριθμούς
#include <algorithm>    
#include <fstream>      //βιβλιοθήκη για χειρισμό αρχείων
#include <chrono>       //βιβλιοθήκη για χρονομέτρηση
...
//συνάρτηση που υπολογίζει τη βέλτιστη διάσταση hypercube
int Hypercube :: hypercube_dimension(const vector<vector<double>>& dataset, int kproj){
    if(kproj > 0)       //αν εχει δοθει kproj χρησιμοποιούμε αυτό
        return kproj;

    int n = dataset.size();
    
    int log_n = static_cast<int>(floor(log2(n)));   //υπολογισμός log2 του μεγέθους dataset
...
//συνάτρηση που αρχικοποιεί LSH hash functions
void Hypercube :: initialize_LSH_func(const vector<vector<double>>& dataset){
    mt19937 gen(seed);                                      //αρχικοποίηση random number generator με το seed
    normal_distribution<double> normal_dist(0.0, 1.0);      //κανονική κατανομή για τα διανύσματα v
    uniform_real_distribution<double> uniform_dist(0.0, w); //ομοιόμορφη κατανομή για τις μετατοπίσεις t
    ...
    // cout << "Hypercube funcs: " << d_prime << "  dimension: " << dimension << endl;

    //δημιουργία LSH funcs για κάθε διάσταση του hypercube
    for(int i=0; i<d_prime; i++){
        vector<double> v_1;
        for(int j=0; j<dimension; j++)
            v_1.push_back(normal_dist(gen));    //τυχαίο διάνυσμα από κανονική κατανομή
        
        v.push_back(v_1);
        t.push_back(uniform_dist(gen));         //τυχαία μετατόπιση από ομοιόμορφη κατανομή
    }
} 

//προβολή ενός σημείου στον hypercube (δημιουργία vertex label)
string Hypercube :: project_to_hypercube(const vector<vector<double>>& dataset, const vector<double>& point){
    ...
    string vertex;
    for(int i=0; i<d_prime; i++){
        //υπολογισμός εσωτερικού γινομένου μεταξύ τυχαίου διανύσματος και point
        ...
        //μετατροπή σε bit (0 ή 1) με βάση το parity
...
//συνάρτηση που κατασκευάζει ευρετήριο hypercube από dataset
void Hypercube :: build_index(const vector<vector<double>>& dataset){       
    initialize_LSH_func(dataset);   //αρχικοποίηση των LSH funcs
    h_map.clear();                  //καθαρισμός υπάρχοντος ευρετηρίου 
    
    this->dataset_reference = dataset;  //αποθήκευση αναφοράς στο dataset για μελλοντική χρήση

    // cout << "hypercube index size: " << dataset.size() << endl;

    //προβολή κάθε σημείου και προσθήκη στον αντίστοιχο bucket
    string vertex;
    for(size_t i=0; i<dataset.size(); i++){     //με int χτυπουσε warning
        vertex = project_to_hypercube(dataset, dataset[i]);
        h_map[vertex].push_back(i);             //προσθήκη point index στο bucket της κορυφής
    }

    /* Στα σχόλια έχει κώδικα με τον οποιο τσεκάραμε τα στοιχε΄΄ια */
...
//συνάρτηση που βρίσκει κορυφές με συγκεκριμένη Hamming απόσταση
vector<string> Hypercube :: vertices_at_hamming_dist(const string& vertex, int target_dist, int max_vertices){  
    vector<string> res;
    int n = vertex.length();
    ...
    //προετοιμασία θέσεων για combinations
    vector<int> positions(n);
    for(int i = 0; i < n; i++)
        positions[i] = i;
    
    //δημιουργία mask για combinations
    ...
    do{    //δημιουργία όλων των συνδυασμών target_dist θέσεων από n
        ...
        //flip τα bits στις επιλεγμένες θέσεις
        for(int i = 0; i < n; i++)
            if(mask[i])
        ...
        if(count >= max_vertices) break;    //σταματάμε αν φτάσαμε το μέγιστο
...
vector<pair<int, double>> Hypercube :: approximate_nearest_neighbors(const vector<double>& query){
    ...
    for(int hamming_dist = 0; hamming_dist <= kproj; hamming_dist++){       //αναζήτηση με αυξανομένη hamming απόσταση 
        auto vertices_at_dist = vertices_at_hamming_dist(query_vertex, hamming_dist, probes - vertices_probed); //εύρεση κορυφών στην τρέχουσα Hamming απόσταση
        
        for(const auto& vertex : vertices_at_dist){
            if(vertices_probed >= probes) break;
            
            if(h_map.find(vertex) != h_map.end()){  //έλεγχος αν η κορυφή υπάρχει στο ευρετήριο
                for(int image_id : h_map[vertex]){
                    if(points_checked >= M) break;
                    
                    double dist = euclidean_distance(dataset_reference[image_id], query);   //υπολογισμός απόστασης και προσθήκη στους υποψήφιους
...
//συνάρτηση που κανει ακριβή αναζήτηση πλησιέστερων γειτόνων
...
vector<int> Hypercube :: approximate_range_search(const vector<double>& query){   //range search
    ...
    for(int i = 0; i <= dim; i++){      //αναζήτηση με αυξανόμενη Hamming απόσταση
...
vector<int> Hypercube :: exact_range_search(const vector<double>& query){
    ...
    for(size_t i = 0; i < dataset_reference.size(); i++){   //έλεγχος όλων των σημείων στο dataset
        double dist = euclidean_distance(dataset_reference[i], query);
...
void Hypercube :: Queries(const vector<vector<double>>& queries,const vector<vector<double>>& dataset){
    ...
    ofstream out(output_file);  //άνοιγμα αρχείου εξόδου
    if(!out.is_open()){
    ...
    for(size_t i = 0; i < queries.size(); i++){
        ...
        if(!aNN.empty() && !eNN.empty()) {
            ...
            //εκτύπωση αποτελεσμάτων για κάθε γείτονα
            ...
            //range search αν είναι ενεργοποιημένη
            vector<int> aRN;
            if(range){
...
//συνάρτηση για την κύρια λειτουργία hypercub
